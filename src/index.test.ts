...
describe('decoder', () => {
  it('works without any option', () => {
    ...
    const string = decoder<string, unknown>({
  ...
  it('works with DecoderFullOption', () => {
    ...
    const alphaNumSchema = string({
      ...
      validate(val, { key } = { key: undefined }) {
        if (val.includes('-'))
          throw new ValidationError(`${key} ${customValidationErrorMsg}`);
      },
  ...
  it('works with CustomErrorMsg', () => {
    ...
    const fooSchema = string(customErrorMsg);
...
describe('object', () => {
  ...
  it('respects custom validator', () => {
    ...
    const customValidator = (val: { password: string; confirm: string }) => {
    ...
    const value1 = [
      {
        name: 'foo',
        mobile: 1234567890,
        address: { postcode: 1, street1: 'street 1', street2: 'street 2' },
      },
    ];
